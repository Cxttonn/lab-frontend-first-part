...
const onSubmit = handleSubmit(async (values) => {
  try {
    const response = await authStore.register(values.firstname, values.lastname, values.email, values.password);
    if (response) {
    ...
    <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
      <form class="space-y-6" @submit.prevent="onSubmit">
        ...
        <div>
          ...
          <InputText v-model="confirmPassword" type="password" placeholder="Confirm Password" :error="errors['confirmPassword']" />
        </div>
        <div>
